#!/usr/bin/env python3
"""
Software License Anomaly Detection - One-Class SVM

This script implements One-Class SVM for detecting suspicious software installations 
...
def main():
    print("=" * 60)
    print("SOFTWARE LICENSE ANOMALY DETECTION - ONE-CLASS SVM")
    print("=" * 60)
    ...
    df_processed['Anomaly_OCSVM'] = anomalies
    
    # Display anomaly statistics
    print("\nAnomaly Detection Results:")
    print(f"Total records: {len(df_processed)}")
    print(f"Anomalies: {df_processed['Anomaly_OCSVM'].sum()}")
    print(f"Normal: {(~df_processed['Anomaly_OCSVM']).sum()}")
    
    # Show some anomalous records
    print("\nSample anomalous records:")
    anomalous_records = df_processed[df_processed['Anomaly_OCSVM'] == True]
    ...
    # Plot normal points
    normal_data = df_processed[df_processed['Anomaly_OCSVM'] == False]
    ...
    anomaly_data = df_processed[df_processed['Anomaly_OCSVM'] == True]
    ...
    plt.title('One-Class SVM: Publisher vs Product Name with Anomaly Detection')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('output/plots/oneclass_svm_scatter.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # Plot count of anomalies
    plt.figure(figsize=(10, 6))
    
    anomaly_counts = df_processed['Anomaly_OCSVM'].value_counts()
    colors = ['#2E8B57', '#DC143C']
    
    bars = plt.bar(['Normal', 'Anomaly'], anomaly_counts.values, color=colors, alpha=0.7)
    plt.title('One-Class SVM: Count of Normal vs Anomalous Records')
    plt.ylabel('Count')
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('output/plots/oneclass_svm_counts.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # Top publishers with anomalies
    publisher_anomalies = df_processed[df_processed['Anomaly_OCSVM'] == True]['Publisher'].value_counts().head(10)
    
    plt.figure(figsize=(12, 6))
    bars = plt.bar(range(len(publisher_anomalies)), publisher_anomalies.values, color='coral', alpha=0.7)
    plt.title('Top 10 Publishers with Most Anomalies (One-Class SVM)')
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('output/plots/oneclass_svm_publishers.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # Decision function scores distribution
    decision_scores = ocsvm_model.decision_function(X_scaled)
    
    plt.figure(figsize=(15, 5))
    
    # Plot 1: Distribution of decision scores
    plt.subplot(1, 3, 1)
    ...
    # Plot 2: Decision scores by class
    plt.subplot(1, 3, 2)
    normal_scores = decision_scores[~anomalies]
    anomaly_scores = decision_scores[anomalies]
    ...
    # Plot 3: Box plot of decision scores
    plt.subplot(1, 3, 3)
    ...
    plt.tight_layout()
    plt.savefig('output/plots/oneclass_svm_scores.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    anomaly_output = df_processed[df_processed['Anomaly_OCSVM'] == True].copy()
    ...
    # Save to CSV
    anomaly_output.to_csv('output/results/Software_Anomalies_SVM.csv', index=False)
    
    print(f"Anomalies saved to 'Software_Anomalies_SVM.csv'")
